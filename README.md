...
```sh
pnpm run task:deployLock --unlock 100 --value 0.1
```
...
To view the coverage report generated by `pnpm run coverage`, just click `Go Live` from the status bar to turn the server
on/off.
